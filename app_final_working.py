...
# Убираем конфликтный route "/" - пусть nginx проксирует напрямую к OHIF
        ...
        function checkAuth() {
            ...
            if (token) {
                if (token === 'valid_admin_token_123') {
                    updateDebug('✅ Токен валидный! Переход к OHIF через правильный путь...');
                    updateDebug('🔄 Используем /ohif/ путь для избежания конфликтов...');
                    ...
                    setTimeout(() => {
                        // Переходим на OHIF path (не корневой)
                        updateDebug('➡️ Переход на /ohif/...');
