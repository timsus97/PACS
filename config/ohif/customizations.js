    ...
    function fetchStudyDataFromOrthanc(studyUID) {
        ...
        fetchStudyDataAsync(studyUID).then(realData => {
            ...
            if (realData) {
                ...
                // Update UI with real data only if still viewing the same study
                if (currentStudyUID === studyUID && document.getElementById('patientInfo').style.display !== 'none') {
                    displayPatientInfo(realData);
                    currentPatientData = realData;
